...
class Game:
    ...
    def update(self):
        if not self.paused:
            ...
            # send the snake to the other side
    ...
    def main_screen(self):
        ...
        if not self.playing:
            ...
            UIElement(14, 12, f"Score: {self.score}").draw(self.screen, 30)
        else:
            UIElement(8, 7, "SNAKE GAME").draw(self.screen, 100)

        UIElement(14, 11, f"High score: {self.high_score if self.high_score > self.score else self.score}").draw(self.screen, 30)
